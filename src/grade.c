...
static void generateComment(Student *stu)
{
    ...
    {
    case 'A':
        strcpy(stu->comment, "学霸认证！继续保持！");
        break;
    case 'B':
        strcpy(stu->comment, "成绩良好，未来可期。");
        break;
    case 'C':
        strcpy(stu->comment, "中规中矩，还需努力。");
        break;
    case 'D':
        strcpy(stu->comment, "低空飘过，注意基础。");
        break;
    case 'F':
        strcpy(stu->comment, "挂科警报！请及时补习。");
        break;
    default:
        strcpy(stu->comment, "暂无有效成绩。");
        break;
    }
}
